#!/usr/bin/env python3
"""
harvest_lobbying.py — the lobbying wire: who paid whom to shape the law,
through what register or none.

After "For Money, Not Law" and the sources its footnotes point to — OECD on
which countries disclose what, OpenSecrets on federal spending and the bills
lobbied for, InfluenceMap on climate policy engagement by companies and trade
associations, Corporate Europe Observatory's RevolvingDoorWatch, the Revolving
Door Project.

What it covers: what is spent and by whom; the lobbyists themselves and how
many there are per legislator; the shadow lobbying that doubles those numbers
because it never registers; the revolving door and the raise that comes with
walking through it; registers and disclosure regimes, and the section's own
figure that only 18 of 195 countries regulate lobbying at all; lobbyists
writing the legislation; access bought with donations, and whose preferences
policy actually tracks; the bodies most lobbied, budget offices first; trade
associations; front groups and astroturf; funded research and captured
expertise; and the sectors the section singles out — climate obstruction,
finance, food and drink, tobacco, pharma, gambling, technology.

A hotel lobby is not this subject, and neither is a company's quarterly
results. Every subject term carries the words it must appear beside.
...
# --------------------------------------------------------------------------
USER_AGENT = ("Mozilla/5.0 (compatible; lobbying-feed/1.0; "
...
# THE SUBJECTS
#
# What is spent, by whom, on whom, through what register or none, and what
# comes out the other end as law.
#
# Each subject is (term, context) pairs. The term must appear AND one of its
# context words, so an ordinary policy story cannot reach the wire.
# --------------------------------------------------------------------------
TOPICS = [
    ("spending", "What is spent", [
        ("lobbying spend*", []), ("lobbying expenditure", []), ("lobbying budget", []),
        ("spent on lobbying", []), ("lobbying bill", ["record", "total", "annual", "rose"]),
        ("lobbying", ["record", "billion", "million", "total", "spending", "rose", "annual", "quarterly"]),
        ("influence spending", []), ("outspent", ["lobby*", "advocates", "opponents", "campaigners"]),
        ("disclosure filing", ["lobbying", "quarterly", "spend*"]),
        ("lda filing", []), ("lobbying report", ["quarterly", "filed", "shows", "record"]),
    ]),
    ("lobbyists", "The lobbyists themselves", [
        ("lobbyist", ["registered", "number", "hired", "former", "employed", "roster", "per", "army"]),
        ("registered lobbyist", []), ("in-house lobbyist", []),
        ("lobbying firm", []), ("government affairs", ["hired", "head of", "office", "team"]),
        ("public affairs firm", []), ("influence industry", []),
        ("lobbying ratio", []), ("lobbyists per", []),
        ("consultant", ["lobby*", "influence", "government relations", "retained"]),
    ]),
    ("shadow", "Unregistered and shadow lobbying", [
        ("shadow lobby*", []), ("unregistered lobby*", []),
        ("failed to register", ["lobby*", "foreign agent", "influence"]),
        ("strategic advice", ["exempt", "unregistered", "loophole"]),
        ("loophole", ["lobbying", "register", "disclosure", "influence"]),
        ("exempt", ["lobbying", "register", "disclosure", "reporting"]),
        ("undeclared", ["lobbying", "meeting", "contact", "influence"]),
        ("informal influence", []), ("influence peddling", []),
    ]),
    ("revolving", "The revolving door", [
        ("revolving door", []), ("cooling-off period", []), ("cooling off period", []),
        ("former minister", ["joined", "hired", "lobby*", "adviser", "board"]),
        ("former official", ["joined", "hired", "lobby*", "adviser", "board", "firm"]),
        ("former regulator", ["joined", "hired", "industry", "firm"]),
        ("former lawmaker", ["lobby*", "hired", "joined", "firm"]),
        ("ex-lobbyist", []), ("appointed", ["from industry", "former lobbyist", "industry executive"]),
        ("secondment", ["industry", "civil service", "department"]),
    ...
    ("registers", "Registers, disclosure and transparency", [
        ...
        ("freedom of information", ["lobby*", "meeting", "influence", "refused", "released"]),
        ("transparency", ["lobbying", "register", "influence", "disclosure", "failure", "gap"]),
        ("under-report*", ["lobbying", "spending", "meeting"]),
        ("late filing", ["lobbying", "disclosure"]),
    ]),
    ("regulation", "Whether it is regulated at all", [
        ("lobbying law", []), ("lobbying rules", []), ("lobbying act", []),
        ("lobbying regulation", []), ("regulate lobbying", []),
        ("lobbying ban", []), ("lobbying reform", []),
        ("code of conduct", ["lobby*", "minister", "official", "breach"]),
        ("ethics commissioner", []), ("ethics watchdog", []), ("standards commissioner", []),
        ("breach", ["lobbying rules", "code", "register", "transparency"]),
        ("sanction", ["lobbying", "register", "breach", "failure to declare"]),
    ]),
    ("drafting", "Who writes the law", [
        ("model legislation", []), ("model bill", []),
        ("drafted by", ["lobby*", "industry", "company", "association", "firm"]),
        ("copy-paste", ["bill", "legislation", "text"]), ("copied text", ["bill", "legislation"]),
        ("industry input", ["bill", "draft", "legislation", "consultation"]),
        ("amendment", ["drafted", "industry", "lobby*", "tabled by", "identical"]),
        ("template", ["bill", "legislation", "law"]),
        ("consultation response", ["industry", "dominated", "majority"]),
    ]),
    ("access", "Access, donations and who gets heard", [
        ("access", ["donor", "lobby*", "minister", "paid", "privileged", "meeting"]),
        ("donor access", []), ("pay to play", []), ("cash for access", []),
        ("fundraiser", ["access", "lobbyist", "donor", "minister", "attended"]),
        ("responsiveness", ["wealthy", "affluent", "donors", "average citizens"]),
        ("preferential", ["access", "treatment", "hearing"]),
        ("hospitality", ["declared", "register", "gift", "minister", "official"]),
        ("gift", ["declared", "register", "official", "minister", "undeclared"]),
    ]),
    ("agencies", "The bodies most lobbied", [
        ("most lobbied", []), ("targeted by lobbyists", []),
        ("budget office", ["lobby*", "influence", "submission"]),
        ("treasury", ["lobby*", "influence", "meeting", "submission"]),
        ("regulator", ["lobby*", "influence", "meeting", "captured", "pressure"]),
        ("agency", ["lobby*", "influence", "meeting", "captured", "pressure"]),
        ("rulemaking", ["comment", "lobby*", "influence", "delayed", "weakened"]),
        ("comment period", ["dominated", "industry", "flooded"]),
        ("regulatory capture", []),
    ]),
    ("tradeassoc", "Trade associations and industry groups", [
        ("trade association", []), ("industry association", []), ("industry body", []),
        ("business lobby", []), ("chamber of commerce", ["lobby*", "opposed", "campaign", "spending"]),
        ("membership", ["trade association", "industry body", "quit", "left", "reviewed"]),
        ("policy footprint", []), ("industry group", ["lobby*", "opposed", "campaign", "spending", "funded"]),
    ...
    ("frontgroups", "Front groups and astroturf", [
        ...
        ("charity", ["funded by industry", "front", "lobbying"]),
        ("coalition", ["industry-funded", "backed by", "front"]),
    ]),
    ("science", "Funded research and captured expertise", [
        ("industry funding", ["research", "study", "science", "trial", "university"]),
        ("funded research", []), ("conflict of interest", ["study", "author", "research", "adviser", "panel", "declared"]),
        ("undisclosed funding", []), ("sponsored study", []),
        ("expert panel", ["industry", "conflict", "member", "adviser"]),
        ("advisory committee", ["industry", "conflict", "appointed", "member"]),
        ("scientific advice", ["industry", "influence", "funded", "pressure"]),
        ("manufactured doubt", []), ("merchants of doubt", []),
        ("nutrition science", ["funded", "industry", "sponsored"]),
    ]),
    ("climate", "Climate policy obstruction", [
        ("climate lobbying", []), ("climate policy engagement", []),
        ("obstruct*", ["climate", "policy", "regulation", "legislation"]),
        ("undermine", ["climate policy", "regulation", "target", "legislation"]),
        ("fossil fuel", ["lobby*", "spending", "influence", "obstruct*", "outspent", "delegates"]),
        ("influencemap", []), ("climate action 100", []),
        ("delegates", ["cop", "climate summit", "fossil fuel", "industry"]),
        ("net zero", ["lobby*", "weakened", "delayed", "opposed"]),
        ("greenwash*", ["lobby*", "policy", "claims", "advert"]),
    ]),
    ("finance", "Financial sector lobbying", [
        ("bank lobby*", []), ("financial lobby*", []),
        ("basel", ["lobby*", "watered", "weakened", "delayed"]),
        ("capital requirement", ["lobby*", "weakened", "delayed", "opposed"]),
        ("financial regulation", ["lobby*", "weakened", "rollback", "delayed", "opposed"]),
        ("insurer", ["lobby*", "opposed", "influence"]),
        ("asset manager", ["lobby*", "opposed", "influence", "policy"]),
        ("crypto", ["lobby*", "spending", "super pac", "influence"]),
    ]),
    ("health", "Food, drink, tobacco, pharma and gambling", [
        ("tobacco lobby*", []), ("alcohol lobby*", []), ("pharma lobby*", []),
        ("food industry", ["lobby*", "opposed", "funded", "influence", "front group"]),
        ("drinks industry", ["lobby*", "opposed", "funded", "influence"]),
        ("beverage", ["lobby*", "tax", "opposed", "funded", "industry"]),
        ("sugar tax", ["opposed", "lobby*", "campaign", "delayed", "dropped"]),
        ("soda tax", []), ("ultra-processed", ["lobby*", "industry", "policy", "regulation"]),
        ("advertising restriction", ["opposed", "lobby*", "delayed", "watered"]),
        ("gambling", ["lobby*", "industry", "opposed", "regulation", "donations"]),
        ("drug pricing", ["lobby*", "opposed", "industry", "campaign"]),
    ]),
    ("tech", "Technology and platform lobbying", [
        ("tech lobby*", []), ("big tech", ["lobby*", "spending", "influence", "opposed", "regulation"]),
        ("platform", ["lobby*", "opposed", "regulation", "influence", "spending"]),
        ("antitrust", ["lobby*", "opposed", "spending", "influence"]),
        ("privacy law", ["lobby*", "opposed", "weakened", "delayed"]),
        ("ai regulation", ["lobby*", "opposed", "influence", "watered", "delayed"]),
        ("search result", ["lobby*", "influence", "boost", "suppress*"]),
        ("data protection", ["lobby*", "opposed", "weakened", "delayed"]),
    ]),
    ("subnational", "States, cities and governments lobbying", [
        ("state government", ["lobby*", "federal", "spending", "hired"]),
        ("local government", ["lobby*", "federal", "spending", "hired"]),
        ("municipal", ["lobby*", "federal", "hired", "spending"]),
        ("intergovernmental lobbying", []),
        ("city hall", ["lobby*", "register", "influence"]),
        ("devolved", ["lobby*", "register", "influence"]),
    ]),
    ("foreign", "Lobbying on behalf of foreign powers", [
        ("foreign agent", ["registration", "act", "charged", "failed to register", "filing"]),
        ("fara", []), ("foreign lobbying", []), ("foreign influence registration", []),
        ("sovereign wealth", ["lobby*", "influence", "hired"]),
        ("state-owned", ["lobby*", "hired", "influence", "firm"]),
        ("embassy", ["hired", "lobby*", "contract", "firm"]),
        ("reputation laundering", []), ("image campaign", ["government", "state", "hired"]),
    ]),
    ("conflicts", "Second jobs and conflicts of interest", [
        ("second job", ["lawmaker", "mp", "legislator", "member", "outside"]),
        ("outside earnings", []), ("outside interests", ["register", "declared", "mp", "lawmaker"]),
        ("register of interests", []), ("declaration of interests", []),
        ("failed to declare", []), ("undeclared interest", []),
        ("shareholding", ["minister", "lawmaker", "declared", "conflict", "divest"]),
        ("recusal", []), ("blind trust", []),
        ("stock trading", ["lawmaker", "congress", "ban", "member"]),
    ]),
    ("reform", "What is set against it", [
        ("lobbying transparency", []), ("mandatory register", []),
        ("legislative footprint", []), ("lobbying ban", []),
        ("revolving door restriction", []), ("cooling-off", ["extended", "introduced", "proposed"]),
        ("oecd", ["lobbying", "transparency", "influence", "principles"]),
        ("investigation", ["lobbying", "influence", "register", "breach"]),
        ("watchdog", ["lobbying", "influence", "transparency", "report"]),
        ("campaign", ["lobbying reform", "transparency", "register", "revolving door"]),
        ("court", ["lobbying", "disclosure", "transparency", "ruling"]),
    ]),
]

ANCHOR = [
    # Influence-machinery language. An ordinary policy story or a company
    # announcement is not this subject; who paid whom to shape the law, through
    # what register or none, is.
    "lobbying", "lobbyist", "lobby register", "lobbying register",
    "transparency register", "lobbying disclosure", "shadow lobbying",
    "unregistered lobbying", "revolving door", "cooling-off period",
    "influence peddling", "regulatory capture", "front group", "astroturf",
    "trade association", "industry association", "model legislation", "model bill",
    "cash for access", "pay to play", "donor access", "register of interests",
    "declaration of interests", "failed to declare", "conflict of interest",
    "foreign agent", "fara", "foreign lobbying", "influencemap",
    "climate lobbying", "climate policy engagement", "climate action 100",
    "ethics commissioner", "standards commissioner", "ethics watchdog",
    "code of conduct", "legislative footprint", "most lobbied",
    "government affairs", "public affairs firm", "lobbying firm",
    "manufactured doubt", "merchants of doubt", "sponsored study",
    "industry funding", "undisclosed funding", "blind trust", "recusal",
]

BLOCK = [
    # the other senses of lobby
    "hotel lobby", "lobby of the building", "in the lobby", "lobby area",
    "lobby card", "lobby group photo",
    # ordinary policy and business coverage without the influence angle
    "quarterly results", "earnings beat", "share price", "market cap",
    "product launch", "hiring spree", "job cuts", "store opening",
    "poll shows", "polling average", "campaign trail", "stump speech",
    "rally in", "debate night", "approval rating", "concession speech",
    # entertainment and filler
    "film review", "movie review", "tv review", "video game", "recipe",
    "gift guide", "coupon", "black friday", "horoscope", "transfer rumour",
...
DECIDED = [
    ...
    "fined", "sanctioned", "suspended", "struck off", "convicted", "charged",
    "ordered", "settled", "resigned", "barred", "referred",
]
INSTITUTIONAL = [
    "oecd", "european commission", "european parliament", "european ombudsman",
    "transparency international", "opensecrets", "influencemap",
    "corporate europe observatory", "sunlight foundation",
    "ethics commissioner", "standards commissioner", "registrar of lobbyists",
    "government accountability office", "national audit office", "auditor general",
    "parliamentary committee", "select committee", "public accounts committee",
    "official gazette", "court filing", "inspector general",
    "peer-reviewed", "published in", "study finds", "working paper", "dataset",
    "official data", "government figures", "national statistics", "disclosure filing",
]
MEASURED = [
    "per cent", "percent", "%", "million", "billion", "record",
    "figures show", "fell by", "rose by", "increase of", "decrease of",
    "estimated", "median", "average", "ranked", "index", "share of", "rate of",
    "number of lobbyists", "meetings held", "spent", "outspent", "ratio",
]
PENDING = [
...
def run(dry_run=False, fixtures=None):
    ...
    payload = {
        ...
        "languages": languages,
        "standings": [
            {"id": "official", "label": "Registrars, regulators & committees"},
            {"id": "research", "label": "Research & influence tracking"},
            {"id": "press", "label": "Press"},
            {"id": "rights", "label": "Transparency & accountability groups"},
        ],
    ...
    if dry_run:
        print("\n--dry-run: wire_lobbying.json not written")
        return payload
